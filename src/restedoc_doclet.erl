...
gen(Sources, App, Modules, Ctxt) ->
    ...
    DocDir = Ctxt#context.redoc_dir,
    Env = Ctxt#context.env,
    Options = Ctxt#context.opts,
    Title = title(App, Options),
    CSS = stylesheet(Options),
    {Modules1, Error} = sources(Sources, Dir, Modules, Env, Options,App,DocDir),
...
sources(Sources, Dir, Modules, Env, Options,App,DocDir) ->
    ...
    {Ms, E} = lists:foldl(fun (Src, {Set, Error}) ->
    			restedoc_apidoc:init(),
				{NewSet, Error1,OverView} = source_comment(Src, Dir, Suffix, Env, Set,Private, Hidden, Error, Options),
				{ok,Result,PathSize} = restedoc_apidoc:generate_doc_json(OverView),
			    case PathSize of 
			    	0 ->
			    		ok;
			    	_ ->
			    		{M, Name, Path} = Src,
			    		FileName = filename:basename(Name, ".erl"),
			    		% io:format("json result:~ts~n", [Result]),
			    		% io:format("will write App:~p redoc_dir:~p~n", [App,DocDir]),
			    		restedoc_lib:write_app_swagger_json(FileName, DocDir, Result),
			    		restedoc_lib:write_module_list(FileName, DocDir, OverView#module_view.title) 
			    end,
			    {NewSet, Error1}
			end,
...
source_comment({M, Name, Path}, Dir, Suffix, Env, Set, Private, Hidden,
       Error, Options) ->
	
	File = filename:join(Path, Name),
	{ok,OverView} = restedoc_comment:comment(File),	
...
source({M, Name, Path}, Dir, Suffix, Env, Set, Private, Hidden,
       Error, Options) ->
    File = filename:join(Path, Name),
    case catch {ok, restedoc:get_doc(File, Env, Options)} of
	{ok,{Module, Doc}} ->
	    check_name(Module, M, File),
	    io:format("source check1:~p~n",[File]),
	    case ((not is_private(Doc)) orelse Private)
		andalso ((not is_hidden(Doc)) orelse Hidden) of
		true ->
		    Text = restedoc:layout(Doc, Options),
		    Name1 = atom_to_list(M) ++ Suffix,
                    Encoding = [{encoding,encoding(Doc)}],
		    restedoc_lib:write_file(Text, Dir, Name1, Encoding),
