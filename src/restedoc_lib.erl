...
-export([count/2, lines/1, split_at/2, split_at_stop/1,
	 ...
	 simplify_path/1, timestr/1, datestr/1, read_encoding/2,write_app_swagger_json/3,
     write_module_list/3]).
...
write_app_swagger_json(File,Dir,Content)->
    FileName = File++".json",
    write_file(Content, Dir, FileName, [{encoding,unicode}]).

write_module_list(FileName,Dir,Title) ->
    ets:insert(api_module, {unicode:characters_to_binary(FileName),unicode:characters_to_binary(Title)}),
    List = ets:tab2list(api_module),
    Content = jiffy:encode({List},[pretty]),
    write_file(Content, Dir, "modulelist.json", [{encoding,unicode}]),
    ok.
