...
-include("restedoc.hrl").
    ...
    % io:format("we're in foo!:~p  ~n appfile:~p~n",[Config,AppFile]).
...
run(Files, Opts0) ->
    ...
    DocDir = proplists:get_value(redoc_dir, Opts0),
    Ctxt1 = Ctxt#context{env = Env,redoc_dir=DocDir},
    % io:format("run get doc_dir:~p~n", [DocDir]),
    % io:format("run Ms2:~p~n Ss1:~p~n App1:~p~n Ctxt1:~p~n",[Ms2,Ss1,App1,Ctxt1]),
    ...
    restedoc_lib:run_doclet(F, Opts).
...
get_redoc_dir(WorkDir, Opt)->
    case proplists:get_value(target_dir, Opt) of 
        undefined ->
            filename:join(WorkDir, "doc");
        {Application,Dir} ->

            T = code:lib_dir(Application),
            filename:join(T, Dir)
    end.
...
target_dir_info(Dir, App, Ms, Opts) ->
	...
	true ->
	    {App, Ms};
	false ->
	    {App1, Ms1} = restedoc_lib:read_info_file(Dir),
	    {if App == ?NO_APP -> App1;
		true -> App
	     end,
	     Ms ++ Ms1}
...
layout(Doc, Opts) ->
    ...
    restedoc_lib:run_layout(F, Opts).
